...
# ------------------ Run Algorithm + GUI Update ------------------
def run_algorithm():
    global tree, xyz_coords
    from script import coords_to_xyz, KDTree, neighbor_cache
    ...
    try:
        cities = get_all_cities(start, start_admin, CSV_FILE)
    ...
    try:
        if algo == "Dijkstra":
            path, distance = dijkstra(graph, cities, start, start_admin, end, end_admin)
        else:
            path, distance = bellman_ford(graph, cities, start, start_admin, end, end_admin)
    except Exception as e:
